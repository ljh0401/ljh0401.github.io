...
export default function CategoryFilter({ selectedCategory }: CategoryFilterProps) {
  ...
  const [mainCategory, subCategory] = selectedCategory ? selectedCategory.split('/') : ['', ''];
  ...
  return (
    <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
      ...
      {mainCategory && !subCategory && currentMainCategory?.subcategories?.map((sub) => (
        <Link
          key={sub.id}
          href={`/blog?category=${mainCategory}/${sub.id}`}
          className={`px-3 py-1 rounded-full text-sm ${
            selectedCategory === `${mainCategory}/${sub.id}` ? 'bg-blue-500 text-white' : 'bg-gray-100 hover:bg-gray-200'
          }`}
        >
          {sub.name}
        </Link>
